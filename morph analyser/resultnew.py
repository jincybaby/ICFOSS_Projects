...
filee=pd.read_csv('change.csv')
myloop2 = filee['letter'].copy()
file1=pd.read_csv('ch.csv')
myloop1 = file1['letter'].copy()
tagfile=pd.read_csv('tags.csv')
tagloop = tagfile['suffix'].copy()

import csv
with open('tag2.csv', 'r') as f:
    reader = csv.reader(f)
...
def tag(i):
    for k in tagloop:
        if i==k:
            n=tagfile.loc[tagfile['suffix']==i].index[0]
            tags=tagfile.loc[n,'tag']
            tagn=[i,"("+tags+")"]
            return (tagn)
    if i in vedict:
        tagn=[i,"(Verb)"]
...
def NV(word,cleanedList):
    if word in vedict:
        lists = [word,"Verb"]
        return lists
    if cleanedList==[] or cleanedList[0] in verdict:
            wor=re.sub("\u0d4d"+'$',"\u0d41"+"ക",word)
            if wor in vedict:
                lists = [wor,"Verb"]
                return lists        
    if cleanedList!=[] and cleanedList[0] in noundict:
        lists = [word,"Noun"]
        return lists
    if word.endswith("\u0d02"):
        lists = [word,"Noun"]
...
def root(word):
    ...
    wordfi=FINAL[0]
    if wordfi.endswith("പ്പെട്"):
        worli=re.sub(" "+'$',"\u0d41"+"ക",wordfi)
        del FINAL[0]
        FINAL=FINAL.insert(0,worli)
